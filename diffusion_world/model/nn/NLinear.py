...
class NLinear(nn.Module):
    ...
    def forward(self, batch):
        pos_shape = batch['position_t'].shape
        input_position = batch['position_t'].view(pos_shape[0], self.position_dim, -1).to(torch.float32)
        x = self.relu(self.position_proj(input_position.squeeze(-1)))
...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, embedding_dim, output_dim, max_positions=1000):
        ...
        self.proj_out = nn.Linear(embedding_dim, output_dim,  bias=False)
